...
public class LoggingServiceClassesAspect {
    ...
    public void callAtAllMyServicesPublicMethods() {
    }

    @Before("callAtAllMyServicesPublicMethods()")
    public void logExecutionTimeBefore() {
        startExecutionTime = LocalDateTime.now();
    }

    @AfterReturning(value = "callAtAllMyServicesPublicMethods()")
    public void logExecutionTimeAfter(JoinPoint joinPoint) {
        String methodName = joinPoint.getSignature().toShortString();
        log.debug("\n"+ANSI_BLUE + "Service: " + methodName +" finished executing in "
                + Duration.between(startExecutionTime,LocalDateTime.now()).toMillis() +" mls" + ANSI_RESET);
    }
